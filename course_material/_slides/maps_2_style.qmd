...
library(scales) # Functions for formatting faster
```
...
-   Generally we don't want to download data every time we run our script
...
-   It is however good to keep a record of the download
-   One option is to 'comment out' the lines of code, but keep them
...
```{r}
ggplot(dat = map_global_fix, aes(x = lon, y = lat)) +
  geom_polygon(aes(group = group), colour = "black", fill = "grey60") +
